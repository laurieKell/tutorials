...
ggplot(melt(dat,id=c("Method","Scenario","year")))+
  geom_polygon(aes(x,y,fill=fill),data=quads) +
  geom_line(aes(year,value,col=factor(Scenario)))+
  facet_grid(variable~Method,scale="free_y", space="free_y",labeller=label_parsed)+
  scale_fill_manual(values=c("pink","lightgreen"), guide="none") +  
  theme_bw()+theme(legend.position="none")+
  ylab(expression(""))+
  xlab("Year")
...
* What do assessment models estimate (NIELS)
  <span style="color:red"> parameters!, which parameters depends on the structure of the model and the data used. What of the quantities below are actually estimated in ASPM, VPA; SS, MFCL, which are derived, and which are nuisance parameters.</span>
    + Fishing mortality
    + Selectivity
    + Stock numbers at age
    + Catchability
    + Residuals <span style="color:red"> what is meant here, goodness of fit statistics, or parameters such as rec or effort deviates from integrated models</span>
    + Uncertainty in the parameters <span style="color:red"> what is meant, value uncertainty, estimation error, If the later how is this derived, bootstrap, MCMC, hessioan, .....</span>
* Where do they differ (LAURIE)
    + Age based assessments
    + Length based assessments
    + Bio-based assessments
    + Data-limted assessments <span style="color:red"> ALl tuna assessments are data limited compared to ICES</span>
    + (LAURIE)
* Example from the tRFMO world (LAURIE)
...
<span style="color:red"> You need to define the types of reference points, i.e recruitment, growth, target, econmic overfishing, then how to derive them</span>
    + SSB - Recruitment
    + Fmsy, Bmsy, Msy
...
<span style="color:red"> We havent even got to day 2 yet</span>    
...
| Day 2         |                                                       |
| --------------|:------------------------------------------------------|
| 09:30 – 10:15 | **Presentation:** Projection scenario                 |
